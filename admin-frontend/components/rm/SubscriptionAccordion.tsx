    ...
    <div className={clsx("overflow-hidden rounded-md border border-outline-variant", open ? "bg-surface-lowest" : "bg-white")}>
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Chip tone={model.tone}>{model.status}</Chip>
          {open
...
function ClientAccordionItem({
  ...
  const allActive = activeCount === client.models.length && client.models.length > 0;
  const headTone = allActive ? "active" : client.models.some((m) => m.tone === "overdue") ? "overdue" : "pending";
    ...
    <section className="overflow-hidden rounded-lg border border-outline-variant bg-surface-lowest shadow-card">
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-3">
          <Chip tone={headTone}>{allActive ? "All active" : `${activeCount}/${client.models.length} active`}</Chip>
          {open
