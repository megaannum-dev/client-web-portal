...
def assert_can_authenticate(user: User, db: Session | None) -> None:
    """Pure login gate (proposal §4.6): raises 401 on disabled account or missing
    ...
    if user.status != AccountStatus.ACTIVE:
        raise HTTPException(401, "Account disabled")  # not yet activated | suspended
    if user.portal == Portal.CLIENT:
        if user.client_profile is None:
            raise HTTPException(401, "Account disabled")  # incomplete record (§4.11 class C)
    else:  # ADMIN
