...
Additive migration (proposal/prompt 006): create the `trades` table — an IB Flex
"Activity" staging table (levelOfDetail=EXECUTION). Same 89 source columns + UUID
PK + ingested_at as `orders` (0004); the column-builder shape is identical so the
two tables share one schema. Each execution's ibOrderID links to its parent order.
...
def _activity_columns() -> list[sa.Column]:
    """91 columns: id PK + 89 source columns + ingested_at. Identical to 0004's
    `orders` (shared schema, see _ActivityRow)."""
    cols: list[sa.Column] = [
