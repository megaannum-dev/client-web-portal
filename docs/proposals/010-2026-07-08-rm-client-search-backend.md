...
- **`components/rm/OnboardingBoard.tsx` is out of scope — not modified by this proposal.** The onboarding-pipeline board is a separate feature whose kanban cards are *prospects* (not necessarily rows in `client_profiles`), so its `KNOWN_CLIENT_IDS`-gated "Open client profile →" navigation belongs to a different data model this proposal does not own. Accepted consequence: once A-6 deletes the `/rm/client-detail/[id]` route, that board's profile link points at a route that no longer exists and 404s — a known, documented regression on an out-of-scope surface, deferred to a future onboarding-board proposal (which will repoint or remove the link when it wires the board to live data). Because that board still imports `KNOWN_CLIENT_IDS`, this proposal also removes **no** export from `lib/mock/rm-data.ts`: `getMockOverlay` is added alongside the existing exports; `RM_CLIENTS`/`CLIENT_EXTRA`/`getClientDetail` become dead-but-kept (harmless unused exports — an unused export trips neither `tsc` nor `next lint`), and `KNOWN_CLIENT_IDS` stays live. Sweeping the dead mock exports is the same follow-up proposal's job.
...
| `admin-frontend/components/rm/OnboardingBoard.tsx` | — | `openProfile()` pushes to `/rm/client-detail/${id}` — **out of scope, not modified.** Its link is deliberately left stale (a known 404 after A-6 deletes the route), deferred to a future onboarding-board proposal (§3 Non-Goals) |
...
- `client-info/page.tsx:103` — `openClient()`, the row-click handler. **In scope** — updated by this finding.
- `components/rm/OnboardingBoard.tsx:151` — `openProfile()`, reached from the onboarding pipeline board. **Out of scope — left untouched.** The board is a separate feature (its cards are onboarding *prospects*, gated by `KNOWN_CLIENT_IDS`, not client-book rows); after the route move its link 404s, which is an accepted, documented regression deferred to a future onboarding-board proposal (§3 Non-Goals). This is a deliberate choice for this revision: repointing that one line would drag an out-of-scope component (and its mock-data coupling) into this proposal's blast radius.
...
2. Update the Client Book's push target (`client-info/page.tsx`) to `/rm/client-info/${id}`. `OnboardingBoard.tsx` is **not** touched (out of scope — its stale link is left as a known 404, per above and §3 Non-Goals).
...
| A-6 | Move `client-detail/[id]` → `client-info/[id]`; delete dead `rm.client-detail` PageId; fix the Client Book's nav push (D-6). OnboardingBoard out of scope — its link left as a known 404 | Yes — user req. | S |
...
2. **Frontend phase — route consolidation (A-6, runs FIRST, before any live-data wiring)** — move `client-detail/[id]/page.tsx` → `client-info/[id]/page.tsx` (still reading mock data at this point — no behavior change beyond location); update the Client Book's hardcoded `/rm/client-detail/${id}` push (`client-info/page.tsx` only — `OnboardingBoard.tsx` is out of scope and left untouched); delete the `rm.client-detail` PageId from `pages-config.ts`; update `pages.check.ts`'s assertions. Verify: `/rm/client-detail/<any-id>` 404s (route no longer exists); `/rm/client-info/<mock-slug-id>` renders the same detail page content as the old route did, byte-for-byte; the Client Book row-click lands on the new path; `OnboardingBoard.tsx` is unchanged and its "Open client profile →" now 404s (known, accepted — §3 Non-Goals); `npx tsx admin-frontend/lib/pages.check.ts` passes.
