...
> **Q-5 outcome (to be filled by BE-13's test run):** `<TODO: record here — "reset link works over a passwordless identity" (FE scope unchanged) or "reset link rejected, email-link fallback in use" (FE gains a set-password form). Dated, one line. The Frontend layer must not be dispatched until this line exists.>`
