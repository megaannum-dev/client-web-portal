...
> **Q-5 outcome (to be filled by BE-13's test run):** `2026-07-30 — decider NOT run: Firebase phase 0 is outstanding in this environment (FIREBASE_AUTH_DISABLED=false, but FIREBASE_CREDENTIALS_PATH=/secrets/firebase.json does not exist — no live Firebase project/credentials reachable here). BE-13's hermetic unit tests (13/13) pass; the marked test_reset_link_over_passwordless_identity decider was deliberately not attempted, per schedule §4's explicit provision for this case. A human with real Firebase credentials must run api-backend/tests/libs/identity/test_set_password_link_type.py::test_reset_link_over_passwordless_identity once and overwrite this line with the actual outcome before the Frontend layer is scheduled.`
